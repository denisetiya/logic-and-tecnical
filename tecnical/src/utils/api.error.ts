import { 
  ...
  const handleApiError = (error: any) => {
    ...
    if (error instanceof PrismaClientKnownRequestError) {
      switch (error.code) {
        ...
        case "P2016":
          throw {
            status: 404,
            message: "Query did not return a result. The requested data was not found.",
          };
        case "P2004":
          throw {
            status: 400,
            message: "A database constraint failed. Check your data integrity.",
          };
        default:
          throw {
            status: 500,
    ...
    // ⚠️ Penanganan error Prisma terkait koneksi database
